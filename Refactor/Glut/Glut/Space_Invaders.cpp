...
int AppSize = 1000, halfsize = AppSize / 2;
float ground = static_cast<float> (halfsize-25);
const int Margin = 15;
const int Anrows = 9, Ancols = 12;
const int moveSpeed = 55;
const int tankSpeed = 210;
const int yDropDistance = 35, yTankLocate = 25;
...
void missileshoot(){
	...
	if (now > wait) {
		...
		wait = now+static_cast<int>(missilePrep*CLOCKS_PER_SEC);
	}
}
void resetGame(){
		for (int row = 0; row < Anrows; row++)
...
void Draw() {
	...
	prevTime = time;
	clock_t now = clock();
	await = now+static_cast<int>(alienAniWait*CLOCKS_PER_SEC); 

	int x = AppSize, y = AppSize;
	int i = 1, h = 1;
	ClearScreen(0,0,0);
